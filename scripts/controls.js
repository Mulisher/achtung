...
function keyPressed(){
    ...
    if(key === ' '){
        switch (state){
            case 'menu': if(!controlsChanging)newGame(); break;
            case 'game': pause = !pause; break;
            case 'round':
                if(checkGameEnd()){ state = 'game_over' }
                else{ newRound() }
                break;
            case 'game_over': backToMenu()
        }
    }
    if(key === 'Escape'){
        switch (state){ case 'game': backToMenu(); break; }
    }
    for(let i = 0; i < 6; i++){
